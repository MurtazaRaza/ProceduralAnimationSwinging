        ...
        public class System
        {
            ...
            public void DrawBones(Color color)
            {
                for (var i = 0; i < bones.Count; ++i)
                    if (bones[i].enabled)
                        DebugDraw.Line(points[bones[i].points[0]].pos, points[bones[i].points[1]].pos, color,
            ...
            {
                var time_sqrd = step * step;
                var acc = new float3(0, Physics.gravity[1], 0);
                for (var i = 0; i < points.Count; ++i)
                {
                    var point = points[i];
                    point.temp = point.pos;
                    if (!point.pinned) point.pos = point.pos + (point.pos - point.old_pos) + acc * time_sqrd;
                }
            }
            ...
            public void EnforceDistanceConstraints()
            {
                for (var i = 0; i < bones.Count; ++i)
                {
                    ...
                    if (num_pinned < 2)
                    {
                        var curr_len = math.distance(points[bone.points[0]].pos, points[bone.points[1]].pos);
                        if (curr_len != 0f)
                        {
                            if (num_pinned == 1)
                            {
                                var pinned = bone.points[1];
                                var unpinned = bone.points[0];
                                ...
                                if (curr_len < bone.length[0])
                                    unpinned_point.pos = points[pinned].pos +
                                                         (points[unpinned].pos - points[pinned].pos) / curr_len *
                                                         bone.length[0];
                            ...
                            else
                            {
                                ...
                                var rel_mass = points[bone.points[1]].mass /
                                               (points[bone.points[0]].mass + points[bone.points[1]].mass);
                                var mid = points[bone.points[0]].pos * (1f - rel_mass) +
                                          points[bone.points[1]].pos * rel_mass;
                                if (curr_len < bone.length[0])
                                {
